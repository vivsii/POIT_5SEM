...
class Program
{
    static int TaskCount = 20; // Количество задач равно количеству логических процессоров
    ...
    static int[,] Matrix = new int[Environment.ProcessorCount, ObservationTime]; // Матрица для сохранения данных
    ...
    static void Main(string[] args)
    {
        ...
        {
            int taskId = i; // Сохраняем текущий идентификатор задачи
            if (i == 0)
            {
                tasks[i] = Task.Run(() => Work(taskId)); // Запускаем первую задачу
            }
            else
            {
                tasks[i] = tasks[i - 1].ContinueWith(t => Work(taskId)); // Запускаем следующую задачу после завершения предыдущей
            }
        }
